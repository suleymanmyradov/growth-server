...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_saved_articles_user_created_desc
    ON saved_articles(user_id, created_at DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_saved_goals_user_created_desc
    ON saved_goals(user_id, created_at DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_saved_habits_user_created_desc
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_habits_user_completed
    ON habits(user_id)
    WHERE completed = TRUE;

-- Bonus: index-only scan support for ClaimDueReminders job queue
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_reminder_queue_due_id
