...
func (l *ResetPasswordLogic) ResetPassword(in *auth.ResetPasswordRequest) (*auth.EmptyResponse, error) {
	...
	entry, exists, err := resetRepo.Get(l.ctx, in.Token)
	if err != nil {
		l.Errorf("failed to lookup reset token: %v", err)
	...
	user, err := l.svcCtx.Repo.Users.GetUserByEmail(l.ctx, entry.Email)
	if err != nil {
	...
	if err := resetRepo.Delete(l.ctx, in.Token); err != nil {
		l.Errorf("failed to delete reset token: %v", err)
	}
