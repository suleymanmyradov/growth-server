...
func (l *ArchiveConversationLogic) ArchiveConversation(in *aicoach.ArchiveConversationRequest) (*aicoach.ArchiveConversationResponse, error) {
	...
	conv, err := l.svcCtx.Queries.ArchiveConversation(l.ctx, convID, userID)
	if err != nil {
		l.Errorf("failed to archive conversation: %v", err)
		return nil, status.Error(codes.Internal, "failed to archive conversation")
	}
