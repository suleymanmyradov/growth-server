...
func (f *fakeIndexer) ListDocIDs(_ context.Context) ([]string, error) {
	return f.publicDocIDs, f.listDocErr
}
func (f *fakeIndexer) ListMemoryDocIDs(_ context.Context) ([]string, error) {
	return f.memoryDocIDs, f.listMemoryErr
}
...
func TestUpsertDocRouting(t *testing.T) {
	...
	if err := s.upsertDoc(context.Background(), "check_in", map[string]any{"id": "check_in:x"}); err != nil {
		t.Fatalf("upsert memory: %v", err)
	}
	if err := s.upsertDoc(context.Background(), "article", map[string]any{"id": "article:y"}); err != nil {
...
func TestDeleteDocRouting(t *testing.T) {
	...
	if err := s.deleteDoc(context.Background(), "conversation_message", memID); err != nil {
		t.Fatalf("delete memory: %v", err)
	}
	if err := s.deleteDoc(context.Background(), "goal", pubID); err != nil {
		t.Fatalf("delete public: %v", err)
	}

	if len(idx.memoryDeletes) != 1 || idx.memoryDeletes[0] != "conversation_message_"+memID.String() {
		t.Errorf("memory delete routed wrong: %+v", idx.memoryDeletes)
	}
	if len(idx.publicDeletes) != 1 || idx.publicDeletes[0] != "goal_"+pubID.String() {
...
func TestUpsertDocPropagatesError(t *testing.T) {
	...
	s := newTestSyncer(idx)
	if err := s.upsertDoc(context.Background(), "weekly_review", map[string]any{}); err == nil {
