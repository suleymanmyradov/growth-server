...
RETURNING id, user_id, plan_id, status, billing_interval, current_period_start, current_period_end, trial_end, cancel_at_period_end, stripe_customer_id, stripe_subscription_id, created_at, updated_at, revenuecat_customer_id, paddle_customer_id, paddle_subscription_id
`
...
const getUserSubscriptionByPaddleCustomerID = `-- name: GetUserSubscriptionByPaddleCustomerID :one
...
type GetUserSubscriptionByPaddleCustomerIDRow struct {
	ID                       uuid.UUID          `db:"id" json:"id"`
	UserID                   uuid.UUID          `db:"user_id" json:"user_id"`
	PlanID                   uuid.UUID          `db:"plan_id" json:"plan_id"`
	Status                   string             `db:"status" json:"status"`
...
// 058 and pkg/paddle.
func (q *Queries) GetUserSubscriptionByPaddleCustomerID(ctx context.Context, paddleCustomerID *string) (GetUserSubscriptionByPaddleCustomerIDRow, error) {
	row := q.db.QueryRow(ctx, getUserSubscriptionByPaddleCustomerID, paddleCustomerID)
	var i GetUserSubscriptionByPaddleCustomerIDRow
	err := row.Scan(
		&i.ID,
		&i.UserID,
		&i.PlanID,
...
const isPaddleEventProcessed = `-- name: IsPaddleEventProcessed :one
SELECT EXISTS(
...
func (q *Queries) IsPaddleEventProcessed(ctx context.Context, eventID string) (bool, error) {
	row := q.db.QueryRow(ctx, isPaddleEventProcessed, eventID)
	var exists bool
	err := row.Scan(&exists)
	return exists, err
}
...
const markPaddleEventProcessed = `-- name: MarkPaddleEventProcessed :exec
INSERT INTO billing_webhook_events (consumer, event_id)
VALUES ('paddle_webhooks', $1)
ON CONFLICT DO NOTHING
`

func (q *Queries) MarkPaddleEventProcessed(ctx context.Context, eventID string) error {
	_, err := q.db.Exec(ctx, markPaddleEventProcessed, eventID)
	return err
}
