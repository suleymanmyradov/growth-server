...
Shared code lives in `pkg/` (ai, analytics, auth, authz, cache, configsafe, email, events, httpx, imageproc, oauth, postgres, prompts, redisutil, server, stripe, validator). Service discovery locally is direct endpoints in the etc yamls (`XxxRpc.Endpoints`, `NonBlock: true`); no etcd in dev.
...
| `services/gateway/contract/swagger/**` | `make swagger-api` | `contract/main.api` |
...
make swagger-api      # regenerate gateway Swagger spec
```
