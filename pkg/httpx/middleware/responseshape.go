...
//
// exemptPaths are endpoints whose responses intentionally do not use the
// standard data envelope (e.g. SSE endpoints). Passing nil disables the
// exemption check.
func ResponseShapeMiddleware(exemptPaths map[string]bool) func(http.HandlerFunc) http.HandlerFunc {
	return func(next http.HandlerFunc) http.HandlerFunc {
		return func(w http.ResponseWriter, r *http.Request) {
			...
			if exemptPaths != nil && exemptPaths[r.URL.Path] {
				next(w, r)
				return
			}
